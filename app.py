...
def save_activity_log(user_id, activity_name):
    """Universal logging for Students, Parents, and Visitors."""
    try:
        ...
        values = (
            user_id if user_id else 0, # Use 0 for Visitors
            activity_name, 
            request.remote_addr,
...
def save_completepr():
    ...
    try:
        cur.execute("""
            INSERT INTO completepr_student
            (user_id, roll_number, admission_number, full_name, date_of_birth,
             phone_number, aadhar_number, course_enrolled, address, city, state,
             pincode, guardian_name, guardian_phone, tenth_school,
             tenth_percentage, emergency_contact)
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
        """, (
            user_id, f["roll_number"], f["admission_number"], f["full_name"],
            f["date_of_birth"], f["phone_number"], f["aadhar_number"],
            f["course_enrolled"], f["address"], f["city"], f["state"],
            f["pincode"], f["guardian_name"], f["guardian_phone"],
            f["tenth_school"], f["tenth_percentage"], f["emergency_contact"]
        ))
        conn.commit()
        session.pop("user_id") 
...
def api_login():
    ...
    if db_role != selected_role:
        return jsonify({"success": False, "message": f"Account is {db_role}, not {selected_role}."})
...
def admin_dashboard():
    ...
    conn = get_db_connection()
    # Using dictionary=True ensures 'log.activity' works in HTML
    cur = conn.cursor(dictionary=True)
    
    # Fetch Student Records for the top table
    ...
    # Fetch ALL Activity Logs for the bottom table (matching your DB screenshot)
...
def parent_view():
    ...
    parent_username = session.get('username')
    conn = get_db_connection()
    cur = conn.cursor(dictionary=True)
    query = "SELECT * FROM completepr_student WHERE guardian_name = %s"
    cur.execute(query, (parent_username,))
...
def update_profile():
    ...
    try:
        cur.execute("""
            UPDATE completepr_student 
            SET full_name=%s, phone_number=%s, course_enrolled=%s, address=%s
            WHERE user_id=%s
        """, (f["full_name"], f["phone_number"], f["course_enrolled"], f["address"], user_id))
        conn.commit()
...
# ---------------- CHATBOT API (INTEGRATED EMAIL) ----------------

@app.route('/api/chat/send', methods=['POST'])
def handle_chat_main():
    # --- UPDATED: Allow Visitors (ID 0) but keep Students/Parents ---
    ...
    # --- NEW FEATURE: Monitor and Store Activity for Marketing ---
    # This records what the user/visitor is looking for in the activity_logs table
    log_detail = f"[{role.upper()}] Chatbot Query: {user_text}"
    save_activity_log(user_id, log_detail)

    conn = get_db_connection()
    cur = conn.cursor(dictionary=True)

    try:
        # --- DEFAULT FEATURE: Get User Profile for Admin Alert ---
        cur.execute("SELECT email, username FROM user WHERE id = %s", (user_id,))
        user_profile = cur.fetchone()
        
        # If visitor, provide dummy profile so email logic doesn't crash
        if not user_profile:
            user_profile = {'email': 'Visitor@Guest', 'username': 'Visitor'}

        # --- DEFAULT FEATURE: Get Student context if exists ---
        cur.execute("SELECT * FROM completepr_student WHERE user_id = %s", (user_id,))
        student = cur.fetchone()
        
        # --- DEFAULT FEATURE: Get AI Response ---
        bot_text, intent, confidence = get_bot_response(user_text, student)
        
        # --- DEFAULT FEATURE: Handle Unknown Query & Email Alert ---
        if intent == "unknown":
            mail_status = send_admin_email(user_profile['email'], user_profile['username'], user_text)
            if mail_status:
                bot_text = f"I'm sorry, I don't know that yet. I've notified the Admin, and they will contact you at {user_profile['email']}."
            else:
                bot_text = "I'm still learning and couldn't answer that. Please contact the office directly."

        # --- DEFAULT FEATURE: Log Conversation in chat_messages table ---
        cur.execute("INSERT INTO chat_messages (sender_id, message, is_ai_response) VALUES (%s, %s, 0)", (user_id, user_text))
        cur.execute("INSERT INTO chat_messages (sender_id, message, is_ai_response, intent, confidence_score) VALUES (%s, %s, 1, %s, %s)", 
                    (user_id, bot_text, intent, confidence))
        
        conn.commit()
        return jsonify({"success": True, "response": bot_text, "intent": intent})
